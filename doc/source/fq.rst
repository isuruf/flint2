...
.. function:: void fq_bit_pack(fmpz_t f, const fq_t op, flint_mp_bitcnt_t bit_size, const fq_ctx_t ctx)

    Packs ``op`` into bitfields of size ``bit_size``, writing the
    result to ``f``.

.. function:: void fq_bit_unpack(fq_t rop, const fmpz_t f, flint_mp_bitcnt_t bit_size, const fq_ctx_t ctx)
